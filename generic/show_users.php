...
if ($count_non_invited) {
    ...
    <?php
    $types_loads_users = get_types_loads_users();
    if ($types_loads_users) {

        foreach ($types_loads_users as $type_loads_users) {
            $type_load_users_id = $type_loads_users;

            ?><option value='<?php echo($type_load_users_id); ?>'
            <?php  if (!empty($show_type_load) && $show_type_load == ($type_load_users_id)) {
            $status_types_load_users_not_changed = 'true'; ?>selected<?php } ?>><?php echo(get_load_type_name_by_id($type_load_users_id)); ?></option>
    ...
    if ($self_load_users_types) {
        ...
        <?php foreach ($self_load_users_types as $self_load_users_type) { ?>
                            ...
                            <?php if (!empty($show_self_load_users_types) && $show_self_load_users_types == $self_load_users_type) {
                                $status_self_load_users_type_not_changed = 'true'; ?>selected<?php } ?>
        ...
        ?>
                    ...
                    <?php foreach ($client_imported_enabled_categories as $client_imported_enabled_category) { ?>
                            ...
                            <?php if (!empty($show_imported_categories) && $show_imported_categories == $client_imported_enabled_category['category_id']) {
                                $status_client_imported_enabled_category_not_changed = 'true'; ?>selected<?php } ?>
